...
Citizen.CreateThread(function()
	...
	if char > 8 then
		print(('[esx_boatshop] [^3WARNING^7] Plate character count reached, %s/8 characters!'):format(char))
	end
end)
...
ESX.RegisterServerCallback('esx_boatshop:resellVehicle', function(source, cb, plate, model)
	...
	if not resellPrice then
		print(('[esx_boatshop] [^3WARNING^7] %s attempted to sell an unknown vehicle!'):format(GetPlayerIdentifiers(source)[1]))
		cb(false)
	else
		local xPlayer = ESX.GetPlayerFromId(source)
